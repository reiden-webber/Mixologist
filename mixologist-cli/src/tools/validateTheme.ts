...
function validateDrinkTheme(
  ...
  const matchingRules = THEME_RULES.filter((rule) =>
    rule.keywords.some((kw) => normalize(theme).includes(normalize(kw))),
  );
...
export const validateThemeTool = new DynamicStructuredTool({
  name: "validate_theme",
  description:
    "Check whether a candidate drink fits the bar's declared theme. " +
  ...
  schema: z.object({
    ...
    drinkStyle: z
      .string()
      .describe(
        "Style descriptors for the drink (e.g. 'tropical, frozen, rum-based')",
      ),
  }),
